...
`prep_training_data.py`: takes a labelled sample of job ads (labelled by Clare). The labelled spans are mapped back to the **sentences** within which they occur and those sentences are the input for model training. Negative examples are generated by taking all the remaining sentences from the job ads in which the labelled spans (and their sentences) occurred. This dataset of job quality sentences and not-job-quality sentences gets split into training, validation and test sets.
